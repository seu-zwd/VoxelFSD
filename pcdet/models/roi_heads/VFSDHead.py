...
# from .roi_head_template_castr import RoIHeadTemplateCasTr
from ..model_utils.transformer_utils import TransformerEncoder, TransformerDecoder, AFB
from ...utils.spconv_utils import spconv


def post_act_block(in_channels, out_channels, kernel_size, indice_key=None, stride=1, padding=0,
                   conv_type='subm', norm_fn=partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)):
...
class VFSDHead(CascadeRoIHeadTemplate):
    def __init__(self, backbone_channels, model_cfg, point_cloud_range, voxel_size, num_class=1, num_frames=1, **kwargs):
        ...
        # self.pos_en = nn.Linear(7, pre_channel)
        # self.trans_encoder = TransformerEncoder(self.model_cfg.TRANSFORMER_CFG)
        # self.trans_decoder = TransformerDecoder(self.model_cfg.TRANSFORMER_CFG)
        c_out_bev = self.model_cfg.BEV_CHANNELS
        self.shared_fc_layer_bevs = nn.ModuleList()
        for i in range(self.stages):
            pre_channel_bev = GRID_SIZE * GRID_SIZE * c_out_bev
            shared_fc_list = []
            for k in range(0, self.model_cfg.SHARED_FC.__len__()):
                shared_fc_list.extend([
                    nn.Linear(pre_channel_bev, self.model_cfg.SHARED_FC[k], bias=False),
                    nn.BatchNorm1d(self.model_cfg.SHARED_FC[k]),
                    nn.ReLU(inplace=True)
                ])
                pre_channel_bev = self.model_cfg.SHARED_FC[k]

                if k != self.model_cfg.SHARED_FC.__len__() - 1 and self.model_cfg.DP_RATIO > 0:
                    shared_fc_list.append(nn.Dropout(self.model_cfg.DP_RATIO))
            self.shared_fc_layer_bevs.append(nn.Sequential(*shared_fc_list))

        self.cls_layers = nn.ModuleList()
        self.reg_layers = nn.ModuleList()
        for i in range(self.stages):
            pre_channel = self.model_cfg.SHARED_FC[-1]*1
            cls_fc_list = []
        ...
        for i in range(self.stages):
            self.cross_attention_layers.append(AFB(self.model_cfg))
        self.init_weights()

    def init_weights(self):
        init_func = nn.init.xavier_normal_
        for module_list in [self.shared_fc_layers, self.shared_fc_layer_bevs, self.cls_layers, self.reg_layers]:
    ...
    def roi_grid_pool(self, batch_dict):
        ...
        roi_grid_xyz = roi_grid_xyz.view(-1, self.pool_cfg.GRID_SIZE ** 3, 3)
        # center_dist = (roi_grid_xyz - rois.view(-1, 1, 7)[:, :, :3]).abs().sum(-1).unsqueeze(-1)
        
        return ms_pooled_features, roi_grid_xyz

    def roi_bev_pool(self, batch_dict):
        rois = batch_dict['rois']
        feats = batch_dict['encoded_spconv_tensor'].dense()
        # feats = batch_dict['spatial_features']
        B, C, H, W = feats.size()
        roi_grid_xy, _ = self.get_global_grid_points_of_bev_roi(
            rois, grid_size=self.pool_cfg.GRID_SIZE
        )  # (BxN, 6x6x6, 3)

        bev_roi_grid_coords_x = (roi_grid_xy[:, :, 0:1] - self.point_cloud_range[0]) / self.voxel_size[0] 
        bev_roi_grid_coords_y = (roi_grid_xy[:, :, 1:2] - self.point_cloud_range[1]) / self.voxel_size[1]

        bev_roi_grid_coords_x /= batch_dict['encoded_spconv_tensor_stride']
        bev_roi_grid_coords_y /= batch_dict['encoded_spconv_tensor_stride']
        bev_roi_grid_coords_x /= W
        bev_roi_grid_coords_y /= H
        bev_roi_grid_coords_x = bev_roi_grid_coords_x * 2 - 1
        bev_roi_grid_coords_y = bev_roi_grid_coords_y * 2 - 1

        samples = torch.cat([bev_roi_grid_coords_x, bev_roi_grid_coords_y], -1)
        samples = samples.view(B, -1, samples.shape[1], 2)
        pooled_features_bev = torch.nn.functional.grid_sample(feats, samples, align_corners=False)
        pooled_features_bev = pooled_features_bev.permute(0, 2, 3, 1).contiguous().view(*rois.shape[:2], -1)
        return pooled_features_bev
    ...
    def get_global_grid_points_of_bev_roi(self, rois, grid_size):
        rois = rois.view(-1, rois.shape[-1])
        batch_size_rcnn = rois.shape[0]

        local_roi_grid_points = self.get_dense_grid_bev_points(rois, batch_size_rcnn, grid_size)  # (B, 6x6x6, 3)
        ...
        global_center = rois[:, 0:3].clone()
        global_roi_grid_points += global_center.unsqueeze(dim=1)
        return global_roi_grid_points[:, :, :2], local_roi_grid_points
    ...
    @staticmethod
    def get_dense_grid_bev_points(rois, batch_size_rcnn, grid_size):
    ...
    def forward(self, batch_dict):
        ...
        # RoI aware pooling
            pooled_features, grid_xyz = self.roi_grid_pool(batch_dict)  # (BxN, 6x6x6, C)
            pooled_features_bev = self.roi_bev_pool(batch_dict)

            b, n, c = pooled_features_bev.shape
            shared_features_bev = pooled_features_bev.view(-1, c)
            shared_features_bev = self.shared_fc_layer_bevs[0](shared_features_bev)
        
            shared_features = self.shared_fc_layers[0](pooled_features.view(pooled_features.size(0), -1))

            _, C = shared_features.size()
            rois_q = shared_features.view(batch_dict['batch_size'], -1, C)
            bev_k = shared_features_bev.view(b, n, -1)

            # all_shared_features.append(shared_features)
            # all_shared_features_bev.append(shared_features_bev)


            rois_features = self.cross_attention_layers[0](rois_q, bev_k, batch_dict['rois'])

            shared_features = shared_features + rois_features.view(-1, C) + shared_features_bev
