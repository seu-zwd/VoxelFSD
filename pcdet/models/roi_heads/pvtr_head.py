...
from ..model_utils.transformer_utils import TransformerEncoder, TransformerDecoder
...
class PVTRHead(RoIHeadTemplate):
    def __init__(self, backbone_channels, model_cfg, point_cloud_range, voxel_size, num_class=1, **kwargs):
        ...
        for k in range(0, self.model_cfg.SHARED_FC.__len__()):
            ...
            if k != self.model_cfg.SHARED_FC.__len__() - 1 and self.model_cfg.DP_RATIO > 0:
                shared_fc_list.append(nn.Dropout(self.model_cfg.DP_RATIO))
        self.shared_fc_layer_bev = nn.Sequential(*shared_fc_list)

        cls_fc_list = []
        pre_channel = self.model_cfg.SHARED_FC[-1]*3
    ...
    def forward(self, batch_dict):
        ...
        shared_features_bev = self.shared_fc_layer_bev(shared_features_bev)
        
        shared_features = self.shared_fc_layer(pooled_features.view(pooled_features.size(0), -1))
        _, C = shared_features.size()
        rois_q = shared_features.view(batch_dict['batch_size'], -1, C)
        bev_k = shared_features_bev.view(b, n, -1)

        pe = self.pos_en(batch_dict['rois'])
        rois_q = rois_q + pe
        bev_k = bev_k + pe

        rois_q = self.trans_encoder(rois_q)
        rois_q = self.trans_decoder(bev_k, rois_q, rois_q)

        # shared_features = shared_features + rois_q.view(-1, C) + shared_features_bev
        shared_features = torch.cat([shared_features, rois_q.view(-1, C), shared_features_bev], -1)
        # shared_features = torch.cat([shared_features, rois_q.view(-1, C)], -1)
        ...
        if not self.training:
            ...
            # batch_dict['rois'] = batch_dict['rois'].unsqueeze(-2).repeat(1, 1, 216, 1).view(4, -1, 7)
            # batch_dict['roi_labels'] = batch_dict['roi_labels'].unsqueeze(-1).repeat(1, 1, 216).view(4, -1)
        else:
