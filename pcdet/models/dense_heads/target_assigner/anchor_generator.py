...
class AnchorGenerator(object):
    ...
    def generate_anchors(self, grid_sizes):
        ...
        for grid_size, anchor_size, anchor_rotation, anchor_height, align_center in zip(
            ...
            anchors = torch.stack((x_shifts, y_shifts, z_shifts), dim=-1)  # [H, W, Ns, 3]
            anchors = anchors[:, :, :, None, :].repeat(1, 1, 1, anchor_size.shape[0], 1)# [H, W, C, Ns, 3]
            anchor_size = anchor_size.view(1, 1, 1, -1, 3).repeat([*anchors.shape[0:3], 1, 1]) 
            anchors = torch.cat((anchors, anchor_size), dim=-1) # [H, W, C, Ns, 6]
            anchors = anchors[:, :, :, :, None, :].repeat(1, 1, 1, 1, num_anchor_rotation, 1)# [H, W, C, Ns, Nr, 6]
            anchor_rotation = anchor_rotation.view(1, 1, 1, 1, -1, 1).repeat([*anchors.shape[0:3], num_anchor_size, 1, 1])
            anchors = torch.cat((anchors, anchor_rotation), dim=-1)  # [H, W, C, Ns, Nr, 7]

            anchors = anchors.permute(2, 1, 0, 3, 4, 5).contiguous() # [C, H, W, Ns, Nr, 7]
            #anchors = anchors.view(-1, anchors.shape[-1])
            anchors[..., 2] += anchors[..., 5] / 2  # shift to box centers
            all_anchors.append(anchors)
        return all_anchors, num_anchors_per_location

    def generate_anchors_sparse(self, grid_sizes, spatial_indices):
        assert len(grid_sizes) == self.num_of_anchor_sets
        all_anchors = []
        num_anchors_per_location = []
        for grid_size, anchor_size, anchor_rotation, anchor_height, align_center in zip(
