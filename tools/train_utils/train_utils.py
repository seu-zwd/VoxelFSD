...
def train_one_epoch(model, optimizer, train_loader, model_func, lr_scheduler, accumulated_iter, optim_cfg,
    ...
    ckpt_save_cnt = 1
    start_it = int(accumulated_iter % total_it_each_epoch)
    ...
    for cur_it in range(start_it, total_it_each_epoch):
        ...
        # average reduce
        avg_data_time = commu_utils.average_reduce_value(cur_data_time)  # data process time
        avg_forward_time = commu_utils.average_reduce_value(cur_forward_time)  # forward compute time
        avg_batch_time = commu_utils.average_reduce_value(cur_batch_time)  # sum of the above
    ...
    epoch_loss /= 3712
    if tb_log is not None:
        tb_log.add_scalar('train/epoch loss', epoch_loss, cur_epoch)
